...
byte; that lives on the parameter set (`params`).
...
from sig_frx.hash.xmss.params import (
    XMSS_PARAMETER_SETS,
...
def _leaf_positions(
...
) -> tuple[list[adrs.Adrs], list[adrs.Adrs]]:
    ...
    """
    return (
        [adrs.ots(position.layer, position.tree, index) for index in indices],
        [adrs.ltree(position.layer, position.tree, index) for index in indices],
    )
...
def leaves(
    ...
    return wots.pk_gen(
        ...
        sk_seed,
        ots_positions,
        wots.ltree_compression(hashes, pub_seed, ltree_positions, leaves=params.len),
    )
...
def sign_layer(
    ...
    signature = wots.sign(hashes, params, message, pub_seed, sk_seed, ots_position[0])
    path = tree.auth_path(
...
def pk_from_sig(
    ...
    ots_positions: list[adrs.Adrs] = []
    ltree_positions: list[adrs.Adrs] = []
