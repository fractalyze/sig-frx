...
- **The addresses are RFC 8391's eight words** (`adrs`), and a component
...
from sig_frx.hash.tweakable import NodeHash, repeat_per_entry
from sig_frx.hash.xmss import adrs
from sig_frx.hash.xmss.adrs import Adrs
from sig_frx.hash.xmss.hashes import Rfc8391Hashes
...
def _chain_addresses(
    ...
    encoded = adrs.encode_batch(Adrs(layers, trees, types, (ots, chains, 0)))
    return [adrs.with_third_word(encoded, step) for step in range(params.w - 1)]
