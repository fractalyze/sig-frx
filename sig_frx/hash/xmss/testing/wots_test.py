...
class _Setup:
    ...
    def __init__(self, oid: int) -> None:
        ...
        n = self.params.n
        self.sk_seed = v.fixture_bytes(n, step=1)
        self.pub_seed = v.fixture_bytes(n, step=2)
        self.message = v.fixture_bytes(n, step=3)
        self.addr = _address(v.ADDR_WORDS)
        self.addr2 = _address(v.ADDR2_WORDS)
...
def _setups() -> dict[int, _Setup]:
    return {oid: _Setup(oid) for oid in v.REFERENCE}
...
class LtreeTest(absltest.TestCase):
    ...
    def test_an_odd_level_lifts_its_last_node_unhashed(self) -> None:
        ...
        compress = wots.ltree_compression(
