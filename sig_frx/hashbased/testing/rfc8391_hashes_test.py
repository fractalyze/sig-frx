...
from sig_frx.hashbased.tweakable import ChainHash, NodeHash
...
def _family(oid: int) -> rfc8391_hashes.Rfc8391Hashes:
    params = rfc8391_params.XMSS_PARAMETER_SETS[oid]
    return rfc8391_hashes.sha2_hashes(params, Sha256())


def _fixture(oid: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """`sk_seed[i] = i`, `pub_seed[i] = 2i`, `m[i] = 3i` — the reference's own."""
    n = rfc8391_vectors.REFERENCE[oid].n
    return (
        rfc8391_vectors.fixture_bytes(n, step=1),
        rfc8391_vectors.fixture_bytes(n, step=2),
        rfc8391_vectors.fixture_bytes(n, step=3),
    )
...
def _core(oid: int, payload: bytes) -> bytes:
    ...
    return hashlib.sha256(payload).digest()[: rfc8391_vectors.REFERENCE[oid].n]
...
def _spec_prf(oid: int, key: bytes, message: bytes) -> bytes:
    ...
    padlen = rfc8391_vectors.REFERENCE[oid].padding_len
...
class ConstructionTest(absltest.TestCase):
    ...
    def test_prf_is_the_keyed_construction(self) -> None:
        for oid in rfc8391_vectors.REFERENCE:
            with self.subTest(oid=oid):
                _, pub_seed, _ = _fixture(oid)
                message = rfc8391_vectors.fixture_bytes(32, start=7)
    ...
    def test_prf_keygen_has_its_own_separator(self) -> None:
        for oid in rfc8391_vectors.REFERENCE:
            with self.subTest(oid=oid):
                ...
                address = a.encode(_address(rfc8391_vectors.ADDR_WORDS))
    ...
    def test_f_keys_and_masks_with_the_address(self) -> None:
        ...
        for oid in rfc8391_vectors.REFERENCE:
            with self.subTest(oid=oid):
                ...
                padlen = rfc8391_vectors.REFERENCE[oid].padding_len
                _, pub_seed, _ = _fixture(oid)
                address = _address(rfc8391_vectors.ADDR_WORDS)
                payload = rfc8391_vectors.fixture_bytes(n, start=1)
    ...
    def test_h_takes_a_mask_twice_as_long(self) -> None:
        ...
        for oid in rfc8391_vectors.REFERENCE:
            with self.subTest(oid=oid):
                ...
                _, pub_seed, _ = _fixture(oid)
                address = _address(rfc8391_vectors.ADDR_WORDS)
                payload = rfc8391_vectors.fixture_bytes(2 * n, start=1)
    ...
    def test_hash_message_binds_the_index(self) -> None:
        for oid in rfc8391_vectors.REFERENCE:
            with self.subTest(oid=oid):
                ...
                padlen = rfc8391_vectors.REFERENCE[oid].padding_len
                sk_seed, pub_seed, _ = _fixture(oid)
                body = rfc8391_vectors.HASH_MESSAGE_BODY
                index = rfc8391_vectors.HASH_MESSAGE_INDEX
    ...
    def test_the_address_is_what_separates_two_identical_inputs(self) -> None:
        ...
        payload = rfc8391_vectors.fixture_bytes(32)
    ...
    def test_prf_matches(self) -> None:
        for oid, vectors in rfc8391_vectors.REFERENCE.items():
            with self.subTest(oid=oid):
                _, pub_seed, _ = _fixture(oid)
                got = _family(oid).prf(
                    pub_seed, rfc8391_vectors.fixture_bytes(32, start=7)
                )
            ...
            with self.subTest(oid=oid):
                ...
                message = np.concatenate(
                    [
                        pub_seed,
                        np.frombuffer(
                            a.encode(_address(rfc8391_vectors.ADDR_WORDS)), np.uint8
                        ),
                    ]
                )
    ...
    def test_f_matches(self) -> None:
        for oid, vectors in rfc8391_vectors.REFERENCE.items():
            with self.subTest(oid=oid):
                ...
                got = _family(oid).f(
                    pub_seed,
                    a.encode_batch(_address(rfc8391_vectors.ADDR_WORDS)),
                    rfc8391_vectors.fixture_bytes(vectors.n, start=1),
                )
        ...
        for oid, vectors in rfc8391_vectors.REFERENCE.items():
            with self.subTest(oid=oid):
                ...
                got = _family(oid).h(
                    pub_seed,
                    a.encode_batch(_address(rfc8391_vectors.ADDR_WORDS)),
                    rfc8391_vectors.fixture_bytes(2 * vectors.n, start=1),
                )
        ...
        for oid, vectors in rfc8391_vectors.REFERENCE.items():
            with self.subTest(oid=oid):
                ...
                got = _family(oid).hash_message(
                    pub_seed,
                    sk_seed,
                    rfc8391_vectors.HASH_MESSAGE_INDEX,
                    np.frombuffer(rfc8391_vectors.HASH_MESSAGE_BODY, np.uint8),
                )
...
class SeamTest(absltest.TestCase):
    ...
    def test_swapping_the_injected_hash_changes_nothing_but_the_hash(self) -> None:
        ...
        payload = rfc8391_vectors.fixture_bytes(32)
        device = rfc8391_hashes.Rfc8391Hashes(Sha256(), n=32, padding_len=32)
        host = rfc8391_hashes.Rfc8391Hashes(HostSha256(), n=32, padding_len=32)
        self.assertEqual(
    ...
    def test_the_padding_length_is_part_of_the_family(self) -> None:
        ...
        # other.
        self.assertNotEqual(
            rfc8391_hashes.Rfc8391Hashes(Sha256(), n=24, padding_len=4),
            rfc8391_hashes.Rfc8391Hashes(Sha256(), n=24, padding_len=32),
        )

    def test_a_truncation_longer_than_the_digest_is_an_error(self) -> None:
        with self.assertRaisesRegex(ValueError, "exceeds"):
            rfc8391_hashes.Rfc8391Hashes(Sha256(), n=64, padding_len=64)
...
class ParameterSetTest(absltest.TestCase):
    def test_the_table_carries_every_registered_oid(self) -> None:
        self.assertEqual(
            sorted(rfc8391_params.XMSS_PARAMETER_SETS), list(range(0x01, 0x16))
        )
    ...
    def test_the_padding_length_is_not_n(self) -> None:
        ...
        by_n = {
            params.n: params.padding_len
            for params in rfc8391_params.XMSS_PARAMETER_SETS.values()
        }
    ...
    def test_names_and_heights_agree(self) -> None:
        self.assertEqual(
            rfc8391_params.XMSS_PARAMETER_SETS[0x01].name, "XMSS-SHA2_10_256"
        )
        self.assertEqual(
            rfc8391_params.XMSS_PARAMETER_SETS[0x0D].name, "XMSS-SHA2_10_192"
        )
        for params in rfc8391_params.XMSS_PARAMETER_SETS.values():
    ...
    def test_a_set_over_the_wrong_core_hash_is_refused(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "SHAKE128"):
            rfc8391_hashes.sha2_hashes(
                rfc8391_params.XMSS_PARAMETER_SETS[0x07], Sha256()
            )
