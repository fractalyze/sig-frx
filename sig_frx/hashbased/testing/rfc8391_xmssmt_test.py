...
from sig_frx.hashbased import rfc8391_params, rfc8391_xmss
...
def _key(oid: int) -> tuple[rfc8391_xmss.Xmss, np.ndarray, np.ndarray]:
    ...
    if oid not in _KEYS:
        ...
        public_key, secret_key = scheme.keygen(
            rfc8391_vectors.fixture_bytes(3 * scheme.params.n)
        )
        _KEYS[oid] = (
            scheme,
...
class ReferenceDigestTest(absltest.TestCase):
    def test_the_public_key_matches(self) -> None:
        for oid, vectors in rfc8391_vectors.XMSSMT_REFERENCE.items():
    ...
    def test_the_index_travels_in_the_signature(self) -> None:
        for oid in rfc8391_vectors.XMSSMT_REFERENCE:
            with self.subTest(oid=oid):
                scheme, _, secret_key = _key(oid)
                signature = np.asarray(scheme.sign(secret_key, _message())[0])
                self.assertEqual(
...
class LayerSplitTest(absltest.TestCase):
    ...
    def test_the_split_matches_the_reference(self) -> None:
        for oid, vectors in rfc8391_vectors.XMSSMT_REFERENCE.items():
            with self.subTest(oid=oid):
                params = rfc8391_params.XMSSMT_PARAMETER_SETS[oid]
                index = rfc8391_vectors.xmss_signing_index(params.height)
                walk = rfc8391_xmss.locate(params, index)
                self.assertEqual(
    ...
    def test_each_layer_is_numbered_by_its_own_depth(self) -> None:
        params = rfc8391_params.XMSSMT_PARAMETER_SETS[0x02]
        walk = rfc8391_xmss.locate(params, 0)
    ...
    def test_the_index_is_consumed_h_prime_bits_at_a_time(self) -> None:
        ...
        params = rfc8391_params.XMSSMT_PARAMETER_SETS[0x02]
        walk = rfc8391_xmss.locate(params, 0b11111)
        ...
        walk = rfc8391_xmss.locate(params, 1 << params.tree_height)
    ...
    def test_single_tree_is_the_one_layer_case(self) -> None:
        ...
        params = rfc8391_params.XMSS_PARAMETER_SETS[0x01]
        walk = rfc8391_xmss.locate(params, 517)
    ...
    def test_an_index_past_the_structure_addresses_a_tree_that_is_not_there(
        ...
        params = rfc8391_params.XMSSMT_PARAMETER_SETS[0x02]
        walk = rfc8391_xmss.locate(params, 1 << params.height)
...
class RoundTripTest(absltest.TestCase):
    ...
    def test_indices_in_different_subtrees_verify_in_one_batch(self) -> None:
        ...
        self.assertLen(
            {rfc8391_xmss.locate(scheme.params, index)[0][0].tree for index in indices},
            3,
        )
        np.testing.assert_array_equal(
            np.asarray(
...
class RejectionTest(absltest.TestCase):
    ...
    def test_an_index_in_another_subtree_is_rejected(self) -> None:
        ...
        original = rfc8391_xmss.locate(params, index)
        walked = rfc8391_xmss.locate(params, moved)
...
class ParameterSetTest(absltest.TestCase):
    def test_the_table_carries_every_registered_oid(self) -> None:
        self.assertEqual(
            sorted(rfc8391_params.XMSSMT_PARAMETER_SETS), list(range(0x01, 0x39))
        )
    ...
    def test_the_two_oid_spaces_are_separate(self) -> None:
        ...
        self.assertEqual(
            rfc8391_params.XMSS_PARAMETER_SETS[0x02].name, "XMSS-SHA2_16_256"
        )
        self.assertEqual(
            rfc8391_params.XMSSMT_PARAMETER_SETS[0x02].name, "XMSSMT-SHA2_20/4_256"
        )
        self.assertNotEqual(rfc8391_xmss.sha2(0x02), rfc8391_xmss.mt_sha2(0x02))
    ...
    def test_the_height_splits_evenly_across_the_layers(self) -> None:
        for params in rfc8391_params.XMSSMT_PARAMETER_SETS.values():
            self.assertEqual(
    ...
    def test_a_height_the_layers_do_not_divide_is_an_error(self) -> None:
        with self.assertRaisesRegex(ValueError, "must divide"):
            rfc8391_params.XmssParams(
                0x99, "made-up", rfc8391_params.CoreHash.SHA2, 32, 32, 20, 3
            )
